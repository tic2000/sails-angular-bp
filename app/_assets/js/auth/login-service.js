...
angular.module('login-service', ['angular-storage'])
.provider('loginService', function () {
  var userToken = localStorage.getItem('sabAppUser'),
      errorState = 'app.error',
      logoutState = 'app.home';

  /*@ngInject*/
  this.$get = function ($rootScope, $http, $q, $state, store, $sailsSocket) {
    userToken = store.get('sabAppUser');
    ...
    var setHeaders = function (token) {
      if (!token) {
        ...
        delete $sailsSocket.defaults.headers.common['X-Token'];
        return;
      }
      else if (token.token) {
        token = token.token;
      ...
      $sailsSocket.defaults.headers.common['X-Token'] = token.toString();
    };

    var setToken = function (token) {
      if (!token) {
        store.remove('sabAppUser');
      } else {
        store.set('sabAppUser', token);
        token = token.token;
      }
      setHeaders(token);
    };
     ...
     */
    var wrappedService = {
      loginHandler: function (response, status, headers, config) {
        /**
        ...
        if (response.user) {
          user = response.user;
          if (user.id) {
            user.userRole = userRoles[user.role];
      ...
      authorize: function (accessLevel) {
        return accessLevels[accessLevel].bitMask & wrappedService.userRole.bitMask;
      },
      /**
       * Public properties
       */
